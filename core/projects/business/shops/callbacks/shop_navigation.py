...
async def shop_main(query: CallbackQuery, bot: Bot):
    text = (
        ...
        "<i>*Это лишь тестовый экземпляр чат-бот магазина, никаких реальных товаров заказать здесь нельзя</i"
    )
...
#     if query.invoice_payload.startswith("phone"):
#         print('ok')
